...
void run_benchmark(void)
{
    ...
    snprintf(curr_dir, MAX_DIR_EXPANSION, "benchmark/");
    ...
    while (run < TOUCH_RM_BENCH) {
        ...
        // Immediately remove the command
    ...
    ESP_LOGI(TAG_BENCH, "TOUCH: ops=%d fails=%d time=%.3fs avg_time=%.3fs throughput=%.2f MiB/s",
             touch.passed, touch.failed, (double)touch.time/1e6, (double)touch.time/(1e6*run),
             0);
    ESP_LOGI(TAG_BENCH, "RM: ops=%d fails=%d time=%.3fs avg_time=%.3fs throughput=%.2f MiB/s",
